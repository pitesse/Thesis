...
This script consumes significance artifacts and comparator outputs and emits:
...
- inferential claims reuse significance tests (two-proportion z as primary, overlap mcnemar as sensitivity),
...
try:
    from .evaluate_significance import KEY_COLUMNS, SCORED_CLASSES
except ImportError:
    from evaluate_significance import KEY_COLUMNS, SCORED_CLASSES
...
DEFAULT_OUTPUT_MD = "data_lake/reports/sde_ml_comparison_2022_2025_merged.md"
DEFAULT_OUTPUT_SUMMARY_CSV = "data_lake/reports/sde_ml_comparison_summary_2022_2025_merged.csv"
DEFAULT_OUTPUT_BY_YEAR_CSV = "data_lake/reports/sde_ml_comparison_by_year_2022_2025_merged.csv"
...
def _load_significance_summary(path: Path) -> pd.DataFrame:
    frame = _load_csv(path, "significance summary")
    required = {
        "model",
        "scored",
        "tp",
...
def _load_significance_tests(path: Path) -> pd.DataFrame:
    frame = _load_csv(path, "significance tests")
    required = {"test", "pairing_scope", "statistic", "p_value", "note"}
    missing = sorted(required.difference(frame.columns))
    if missing:
        raise ValueError(f"significance tests missing columns: {missing}")
    ...
    if not expected_tests.issubset(found_tests):
        raise ValueError(
            "significance tests missing required rows: "
...
def _build_markdown(
    ...
    lines.append("## Inputs")
    lines.append(f"- Significance summary: {significance_summary_path}")
    lines.append(f"- Significance tests: {significance_tests_path}")
...
def main() -> None:
    ...
    significance_summary_path = Path(args.significance_summary)
    significance_tests_path = Path(args.significance_tests)
    ...
    summary_df = _load_significance_summary(significance_summary_path)
    tests_df = _load_significance_tests(significance_tests_path)
    ...
    # keep inferential values pinned to significance outputs, comparators are used for descriptive diagnostics.
    summary_row = {
    ...
    markdown = _build_markdown(
        ...
        significance_tests_path=significance_tests_path,
    ...
    print("=== DEDICATED SDE VS ML COMPARISON ===")
    print(f"significance summary    : {significance_summary_path}")
    print(f"significance tests      : {significance_tests_path}")
