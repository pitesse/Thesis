...
the goal is to compare the behaviour of the Flink Strategy Engine, Batch XGBoost, and the MOA OzaBoostADWIN on the same pit decision contracts 
\cite{chen2016xgboost,bifet2010moa,oza2005boosting,bifet2007adwin}.
...
in prequential order \cite{chen2016xgboost,bifet2010moa,oza2005boosting,bifet2007adwin}. The protocol defines how these outputs are converted into comparable positive calls and 
how those calls are matched against truth,
all headline results are computed on seasons 2022--2025 which is the ground effect F1 car era. The truth universe mode is \texttt{canonical\_sde\_truth}, and the main lens is 
\texttt{clean\_actionable}, this means
...
\cite{roberts2017cvstructured}.
The MOA systems are evaluated with the prequential test-then-train protocol where each row is predicted first and then used for learning \cite{bifet2010moa,oza2005boosting}. 
The Flink Strategy Engine is evaluated from its persisted \texttt{pit\_suggestions} artifacts, with the selected frozen rule profile.
...
fitting makes the Batch branch slower \cite{roberts2017cvstructured}. MOA instead runs once over the exported stream in prequential order, each instance is predicted and then 
used for training, so the learner processes the stream in a single pass \cite{bifet2010moa,oza2005boosting}.
...
XGBoost against MOA since in this pipeline MOA is faster mainly because it uses a single prequential pass, while Batch XGBoost repeatedly fits models across
...
but only 5,855 of them are positive rows, which means that positive pit-timing situations represent about 6.3\% of the learner rows. A low absolute precision is
...
The 5,855 positive rows are not 5,855 distinct pit stops, they are positive driver-lap examples produced by the horizon label; intuitively, one
...
recall. The ratio between row positives and unique pit events is about $5,855/3,048=1.92$, close to but not exactly $2.00$, this is expected because some pits occur
...
Before presenting the \texttt{pit\_success\_h2} results, the accounting semantics must be clarified since the success contract is stricter than the pit timing contract because a
positive call is correct only if an eligible future pit happens and that pit is later classified as successful.
This creates a distinction between decision-level quantities and event-level quantities because a system emits positive calls at driver-lap level, while the truth is defined by
...
The following sections use strict precision and successful event coverage for the main \texttt{pit\_success\_h2} comparison while keeping matched-pit success rate separate as
...
The relaxed rows are diagnostic only since they are not the main headline policy since their purpose is to show the trade-off between call volume, strict precision, and successful
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.99\textwidth]{Images/ch5/pit_success_apples_to_apples_main.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.99\textwidth]{Images/ch5/pit_success_sde_diagnostic_operational_vs_matched.png}
    \caption{Flink Strategy Engine diagnostic matched-pit success rate compared with strict operational precision.}
...
The two headline tasks show different behaviours, the \texttt{pit\_any\_h2} contract is the cleaner timing task since it only asks whether a pit stop happens soon,
...
The Flink Strategy Engine is different since it does not produce a continuous learned score used for threshold sweeping in the same way as Batch and MOA. It emits deterministic
...
The purpose of this section is to inspect whether the learned systems rank the rows in a useful way, the comparison remains based on the selected thresholds and on
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.99\textwidth]{Images/ch5/pr_curve_pit_any_final.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.99\textwidth]{Images/ch5/pr_curve_pit_success_final.png}
...
\begin{table}[htbp]
    \centering
    \caption{Average Precision for score-based systems. AP is a row-level ranking diagnostic.}
...
The AP table should be read together with the headline metrics since a model can have a useful ranking curve while still requiring a conservative threshold
...
Kappa measures agreement with the target after accounting for agreement that could happen by chance while G-Mean combines sensitivity and specificity,
so it is useful when the positive class is rare \cite{cohen1960kappa,he2009imbalanced}.

A support filter is applied before plotting the race-level values since races with too few positives can produce unstable diagnostic scores,
...
Figures~\ref{fig:race_level_kappa_pit_any} and~\ref{fig:race_level_gmean_pit_any} report race-level diagnostics for the \texttt{pit\_any\_h2} task where
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.90\textheight,keepaspectratio]{Images/ch5/race_level_kappa_pit_any.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.90\textheight,keepaspectratio]{Images/ch5/race_level_gmean_pit_any.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.90\textheight,keepaspectratio]{Images/ch5/race_level_kappa_pit_success.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.90\textheight,keepaspectratio]{Images/ch5/race_level_gmean_pit_success.png}
...
The MOA Online Learner is harder to inspect directly since the model is executed as a streaming learner and the implementation does not expose the same type of native feature 
...
as shortcuts, and whether the engineered profiles change the information available to the learner.
...
Figure ~\ref{fig:batch_shap_top_feature_comparison} show the Batch explainability evidence used during this correction phase. 
The beeswarm plots in figure ~\ref{fig:batch_shap_beeswarm_no_year} and ~\ref{fig:batch_shap_beeswarm_percent} compare the attribution patterns of the original and 
Percent style Batch configurations. The feature comparison plot summarizes how the most relevant 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{Images/ch5/shap_beeswarm.png}
    \caption{Batch original feature view.\label{fig:batch_shap_beeswarm_no_year}}
\end{figure}
\FloatBarrier

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{Images/ch5/phase2b_batch_p1_shap_beeswarm.png}
    \caption{Batch Percent feature view.\label{fig:batch_shap_beeswarm_percent}}
\end{figure}
\FloatBarrier
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Images/ch5/phase2b_batch_shap_top_feature_comparison_e0_p1_top.png}
    \caption{Top Batch SHAP features before and after the Percent profile correction.}
    \label{fig:batch_shap_top_feature_comparison}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Images/ch5/phase2b_batch_shap_top_feature_comparison_e0_p1_delta.png}
    \caption{Change in top Batch SHAP feature importance after the Percent profile correction.}
    \label{fig:batch_shap_delta_feature_comparison}
\end{figure}
